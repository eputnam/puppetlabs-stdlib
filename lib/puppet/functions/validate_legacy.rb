Puppet::Functions.create_function(:validate_legacy) do
  ...
  dispatch :validate_legacy do
    param _('Any'), :scope
    param _('Type'), :target_type
    param _('String'), :function_name
    param _('Any'), :value
  ...
  def validate_legacy(scope, target_type, function_name, value, *prev_args)
    if assert_type(target_type, value)
      ...
      error_msg = Puppet::Pops::Types::TypeMismatchDescriber.new.describe_mismatch(_("validate_legacy(#{function_name})"), target_type, inferred_type)
      if previous_validation(scope, function_name, value, *prev_args)
        call_function(_('deprecation'), _('validate_legacy'), error_msg)
      else
        call_function(_('fail'), error_msg)
      end
    end
  end
  ...
  def previous_validation(scope, function_name, value, *prev_args)
    ...
    begin
      scope.send(_("function_#{function_name}").to_s, [value, *prev_args])
      true
